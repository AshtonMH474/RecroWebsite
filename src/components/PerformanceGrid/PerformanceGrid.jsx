...
function PerformanceGrid({ performanceRes, ...block }) {
  ...
  return (
    <>
      <div id={block.performance_id} className="relative bg-black overflow-hidden w-full pb-24">
        ...
        <div className="pt-12 flex flex-wrap justify-center gap-x-6 gap-y-12 max-w-[1500px] mx-auto">
          {performances?.map((performance, i) => (
            <PerformanceCard
              ...
              key={`${performance._id || performance.title}-${i}`}
              onExpand={() => openCard(i)}
            />
          ))}
        </div>
      ...
      <AnimatePresence>
        {expandedCardIndex !== null && (
          <PdfModal solution={performances[expandedCardIndex]} onClose={closeCard} />
        )}
      </AnimatePresence>
    </>
  );
