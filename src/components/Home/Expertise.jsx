...
const Expertise = forwardRef(function Expertise(props, ref) {
  ...
  // Effect to calculate number of rows based on screen width
  useEffect(() => {
    const updateRows = () => {
      const screenWidth = window.innerWidth;
    ...
    window.addEventListener("resize", updateRows); // Listen to resize for row calculation

    return () => window.removeEventListener("resize", updateRows);
  }, [expertiseItems.length]); // Re-run if number of expertise items changes

  // Effect to calculate section height based on rows and stable viewport height
  useEffect(() => {
    const screenWidth = window.innerWidth;

    // Get the stable --vh value from the root element's computed style
    // Fallback to window.innerHeight if --vh isn't set (though it should be by _app.js)
    const vhValue = parseFloat(getComputedStyle(document.documentElement).getPropertyValue('--vh')) || (window.innerHeight * 0.01);
    const stableViewportHeight = vhValue * 100; // Convert vh to pixels

    const cardsPerRow = screenWidth < 640 ? 1 : screenWidth < 948 ? 2 : 3;
    const isShort = stableViewportHeight <= 600;
    const isTall = stableViewportHeight >= 1000;

    setShort(isShort);
    setTall(isTall);

    const stickyMultiplier = cardsPerRow === 1 ? 1.5 : 1;
    // Base these calculations on the stableViewportHeight
    const rowHeightPx = 0.8 * stableViewportHeight;
    const headingHeightPx = 0.8 * stableViewportHeight;
  ...
  return (
    <>
      <section
        ref={sectionRef}
        // Applying the calculated height directly.
        // No need for 'full-screen-section' class here as height is dynamic based on content and rows.
