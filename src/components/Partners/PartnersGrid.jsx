...
export default function PartnersGrid({ partnersRes, ...block }) {
  ...
  const allPartners =
    partnersRes?.partnerConnection?.edges.map((e) => e.node) || [];
  ...
  const filtered = useMemo(() => {
    ...
    if (category && category !== "All") {
      return allPartners.filter(
        (partner) => partner?.category?.category === category
      );
    }
    return allPartners;
  ...
  return (
    <div
      id={block.partners_id}
      className="relative bg-black w-full overflow-hidden pb-24"
    >
      {/* Heading Section */}
              ...
              >
                ...
                >
                  ...
                  {partner.needsTitle && partner.title && (
                    <h1 className="font-bold text-[26px] text-center">
                      {partner.title}
                    </h1>
                  )}
                </div>
