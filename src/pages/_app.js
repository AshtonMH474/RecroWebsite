...
export default function App({ Component, pageProps }) {
  ...
  // ✅ Memoized mobile Safari detection
  ...
  // ✅ Lock height only on iOS Safari
  useEffect(() => {
  ...
  // ✅ Initial --vh setup for all non-Safari browsers
  useEffect(() => {
    if (isMobileSafari) return;
  ...
  // ✅ Resize listener for --vh (non-Safari only)
  useEffect(() => {
    if (isMobileSafari) return;
  ...
  return (
    <>
      ...
      <div
        style={{
          height: isMobileSafari
            ? lockedHeight
              ? `${lockedHeight}px`
              : "100vh"
            : "auto",
          minHeight: "100vh", // ✅ fallback for SSR safety
        }}
      >
        <Component {...pageProps} />
      </div>
