...
export default function App({ Component, pageProps }) {
  const lastSize = useRef({ width: 0, height: 0 });
  const [isTransformActive, setIsTransformActive] = useState(true);
  ...
  useEffect(() => {
    requestAnimationFrame(() => {
      ...
      // Remove transform after 300ms (adjust timing as needed)
      setTimeout(() => {
  ...
  return (
    <>
      ...
      {/* Conditionally add the transform class */}
      <div className={`app-wrapper ${isTransformActive ? 'with-transform' : ''}`}>
        <BG />
        <Component {...pageProps} />
      </div>
    </>
